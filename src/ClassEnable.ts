...
export default function ClassEnable<T>(Comp: T, displayName?: string): T {
	...
	class ClassEnableInner extends React.Component {
		...
		getInnerMyStyle = (path, styles, id) => {
			const { styles: oldStyles, id: oldId, path: oldPath, result } = this.cacheInfo.styleCache;
		...
		render() {
			...
			// @ts-ignore
			return React.createElement(Comp, { ...this.props, style: [styleN, this.props.style] });
		}
	}
